...
\subsection{Approximating\, \footnotesize$\mathcal{C}_{\theta}(e) \defeq \mathcal{K}(G\bsk e)$\,\small\ in\,\ 
	\footnotesize$\tilde{O}(m)$\,\small\ time}
...
\frame{
	...
	\begin{enumerate}	
		...
		\item \textbf{Cholesky factorization} \,of \,\footnotesize$\LL=\matlow\calD\matlow^T$\,\small\ 
			...
			\[
				\vspace{-8pt}
				\LL = \SS^{(n)} + \sum\limits_{i=1}^{n-1} \alpha_i \cc_i\cc_i^T =
...
\frame{
	...
	\begin{itemize}
		...
		\item \scriptsize$\SS^{(1)}$\,\footnotesize\ is a Laplacian
...
\frame{
	\frametitle{Clique Sampling\footnote[frame]{R. Kyng and S. Sachdeva. \textbf{FOCS`16}.}}
	\vspace{5pt}
	\begin{itemize}
		\small
		\item Graph sparsification
			\begin{itemize}
				\item \footnotesize Sampling edges by \scriptsize$w(e)\er(e)$\footnotesize\,\ leads to an
					\scriptsize$\tilde{O}(n)$\footnotesize-edge \scriptsize$\LH \approx_\eps \LL$\footnotesize\,\ since
					\scriptsize\vspace{-8pt}
					\[
						\vspace{-8pt}
				...
				\item Sampling by \scriptsize$\ttau_e \geq w(e)\er(e)$\footnotesize\,\ leads to an
					\scriptsize$\tilde{O}(\sum\nolimits_{e\in E}\ttau_e)$\footnotesize-edge \scriptsize$\LH \approx_\eps \LL$\footnotesize
			\end{itemize}
		\item Sparsifying \,\footnotesize$\SS^{(1)} = \LL_{G[V\setminus v_1]} + \sum\limits_{i\sim 1}\sum\limits_{j\sim 1} \frac{w(1,i)w(1,j)}{d}\bb_{i,j}\bb_{i,j}^T$\small
		\begin{itemize}
...
\frame{
	\frametitle{Approximate Gaussian Elimination for Laplacians}
	\begin{itemize}
		\item Eliminate vertices in random order
			\begin{itemize}
				\item Every step eliminate a vertex with average degree in expectation
				\item Total elimination time and total nonzero entries in factorization are both
					\footnotesize
					\[
						\frac{m}{n} + \frac{m}{n-1} + \ldots + \frac{m}{1} = O(m\log n)
					\]\small
				\item Thereby obtain an approximate Cholesky factorization \footnotesize$\LL\approx_\eps \matlow\calD\matlow^T$  
					\small\ with \footnotesize$\tilde{O}(m)$\small\ nonzero entries in \footnotesize$\tilde{O}(m)$\small\ time
			\end{itemize}
		\item For any vector \footnotesize$\zz \in \mathbb{R}^n$\small, we can compute in \footnotesize$\tilde{O}(m)$\small\ time
...
\frame{
	...
	\begin{itemize}
		...
		\begin{enumerate}
			\item Compute in \footnotesize$\tilde{O}(m)$\,\small\ time\, \footnotesize$\LL \approx_\eps \matlow \calD \matlow^T$
				with \footnotesize$\tilde{O}(m)$\,\small\ nonzero entries
...
\frame{
	...
	\begin{table}\caption{Algorithms that use div-conquer}
		...
		\begin{tabular}{c|c|c}
			...
			\begin{tabular}{c}
					%\hline
					Exact \scriptsize$\tilde{O}(n^{2.37})$\footnotesize\footnote[frame]{V. V. Williams. \textbf{STOC`12}.} \\
						%\footnote[frame]{D. Durfee, R. Kyng, J. Peebles, A. B. Rao, and S. Sachdeva. \textbf{STOC`17}.} \\
					%\hline
					%\vspace{-5pt}
					Approx. \scriptsize$\tilde{O}(n^{2})$
					\footnote[frame]{D. Durfee, J. Peebles, R. Peng, and A. B. Rao. \textbf{FOCS`17}.}
				\end{tabular}
			& \scriptsize$\tilde{O}(n^2)$\footnotesize \footnotesize\footnotemark[3]
				 \\
			\hline
			Rand spanning tree &
				\begin{tabular}{c}
					...
					%\hline
					%\vspace{-5pt}
					Approx. \scriptsize$\tilde{O}(n^{2})$\footnotesize\footnotemark[3]
				\end{tabular}
				&
			...
			Approx. maxflow & \scriptsize$\tilde{O}(m)$\footnotesize
				...
				\scriptsize$\tilde{O}(m)$\footnotesize\footnotemark[6] \\
			\hline
			\scriptsize$\LL\xx = \bb$\footnotesize & \scriptsize$\tilde{O}(m)$\footnotesize
				...
				\scriptsize$\tilde{O}(m)$\footnotesize\footnotemark[7] \\
			\hline
		\end{tabular}
	\end{table}
	%\end{itemize}
...
\subsection{Approximating\, \footnotesize$\mathcal{C}_{\theta}^\Delta(e) \defeq \mathcal{K}(G\bsk e) - \mathcal{K}(G)$\,\small\ in\,\ 
	\footnotesize$\tilde{O}(m)$\,\small\ time}
...
\subsection{Approximating\, \footnotesize$\mathcal{C}_{\theta}^\Delta(v) \defeq \mathcal{K}(G\bsk N(v)) - \mathcal{K}(G)$\,\small\ in\,\ 
	\footnotesize$\tilde{O}(m)$\,\small\ time}
