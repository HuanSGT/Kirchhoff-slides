	...
	\begin{itemize}
		...
		%	\begin{itemize}
				\item Identify influential persons in a social network
					\begin{itemize}
						\item Independent cascade model: measuring the importance of a person by its ability of information
							spreading
					\end{itemize}
...
\frame{
	...
	\begin{itemize}
		...
		%\item Common used edge centrality for an edge \small $e \in E$ \normalsize
		  ...
		  \item Weakness
		  	\begin{itemize}
		  		\item Betweenness: only considering shortest paths
...
\frame{
	...
	\begin{itemize}
		...
		\item Connection to commute times in random walks
			\begin{itemize}
				\item \small The commute time \scriptsize$\kappa(s,t)$\, \small between
					\scriptsize$s,t$ \,\small is the expected time of a random walk to go from
					\scriptsize$s \to t \to s$\small, and it is known that
					\vspace{-12pt}
					\scriptsize
					\[
					...
					\]
					\small
					where \scriptsize$\mathrm{vol}(G)$ \small is the sum of degrees over all vertices
				\item Thus, \scriptsize$\er(s,t)$ \,\small takes into account all paths between \scriptsize$s,t$
			\end{itemize}
	\end{itemize}
}
...
\frame{
	...
	\end{figure}
	\begin{itemize}
		\item Betweenness cannot distinguish between \small$e_1$\normalsize\ and \small$e_2$\normalsize
				\small
				\vspace{-15pt}
				\begin{align*}
				...
				\end{align*}
				\vspace{-30pt}
		\item Spanning edge centrality cannot distinguish between \small$e_3$\normalsize\ and \small$e_4$\normalsize
				\small
				\vspace{-15pt}
				\begin{align*}
