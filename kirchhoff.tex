...
\frame{
	...
	\begin{itemize}
		\footnotesize
		\item \scriptsize$\AA \pleq \BB \quad\Leftrightarrow\quad \forall \xx\in\mathbb{R}^n,\ \xx^T\AA\xx \pleq \xx^T\BB\xx$\footnotesize\
		...
		\item Properties
			\begin{enumerate}
				...
				\AA \approx_{\eps_1 + \eps_2} \BB + \DD$\footnotesize
				\item \scriptsize$\AA\approx_\eps \BB \quad \Rightarrow \quad 
		...
		\item {\color{red} \scriptsize$\AA\approx_\eps \BB, \CC\approx_\eps \DD \quad \centernot\Rightarrow \quad 
			...
			\vspace{-5pt}
			\begin{itemize}
				\item {\color{red} \scriptsize$91\approx_{0.1} 100, 90\approx_{0.1} 90$\footnotesize\ \,
					but
					\ \scriptsize$1 \centernot\approx_{0.1} 10$\footnotesize }
			\end{itemize}
	\end{itemize}
}
		...
		\item Eliminate vertices in random order
			\begin{itemize}
				...
				\item Thereby obtain an approximate Cholesky factorization \footnotesize$\LL\approx_\eps \matlow\calD\matlow^T$  
					\small\ with \footnotesize$\widetilde{O}(m/\eps^2)$\small\ nonzero entries in \footnotesize$\widetilde{O}(m/\eps^2)$\small\ time
			\end{itemize}
		\item \small $\widetilde{O}(m) \defeq O(m\ \mathrm{poly}(\log n))$\normalsize\ \,is called nearly linear time
		\item For any vector \footnotesize$\zz \in \mathbb{R}^n$\small, we can compute in \footnotesize$\widetilde{O}(m/\eps^2)$\small\ time
...
\frame{
	...
	\begin{block}{\small$\textsc{QuadEst}\footnotesize(\LL, \zz, E_Q = E)$: \small Estimate\ \footnotesize$\zz^T(\LL\bsk e)^\dag\zz$\hspace{4pt}\small for all \footnotesize$e\in E_Q$}
		\begin{enumerate}
			...
			\item Eliminate vertices in \scriptsize$F$\footnotesize\ in random order in nearly linear time:
			\scriptsize\vspace{-8pt}
			\[\vspace{-8pt}
				\LL^\dag \approx_{\eps} 
				\begin{pmatrix}
			...
			\[
				\vspace{-12pt}
				\zz^T\LL^\dag\zz = \yy_F^T \calD^{-1}\yy_F + \yy_C^T \SS^\dag \yy_C
			\]
			\footnotesize
